...
const dayTypeValidator = (props, propName, componentName) => {
  ...
  if (
    !_.some(
      DAYS,
      day => day.toLowerCase() === prop || `${day}s`.toLowerCase() === prop,
    )
  ) {
    return new Error(
...
const getDayIndex = day =>
  _.findIndex(
    DAYS,
    test =>
      day.trim().toLowerCase() === test.toLowerCase() ||
      day.trim().toLowerCase() === `${test}s`.toLowerCase(),
  );
...
class WeekCalendar extends PureComponent {
  ...
  static eventRenderer = event => (
    <div className="default-calendar-event calendar-event">{event.content}</div>
  );
  ...
  getHeaders = defaultMemoize(
    (days, weekStart, start, end, interval, precision) => {
      ...
      return _.map(_.range(days), day => [
        ...
        >
          <p className="center-content">{DAYS[(startIndex + day) % 7]}</p>
        </div>,
        <div
  ...
  getRowHeaders = defaultMemoize(
    (start, end, interval, numDays, renderer, precision) =>
  ...
  getEventElements = defaultMemoize(
    (start, events, renderer, weekStart, numDays, precision) => {
            ...
            dayPos < numDays && (
              <div
                ...
                style={{
                  ...
                  gridRowStart:
                    this.gridify(event.start - start, precision) + 2,
