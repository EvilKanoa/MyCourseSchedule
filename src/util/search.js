...
class Search {
  constructor(items, indexFields) {
    ...
    this.haystack = this.items.map((item, index) => {
      const indexed = {
        ...item,
  ...
  search = (
    ...
    const safeSort = [...options.sort].filter(
      opt => opt.field && opt.direction && opt.weight,
    );
    safeSort.forEach(sort => {
      if (sort.weight !== 0) {
        ...
        sorted.forEach((item, sortIndex) => {
          weights[item.__search_data_index__] +=
            ((this.haystack.length - sortIndex) / this.haystack.length) *
            sort.weight;
        });
      }
    });
    ...
    let results = this.haystack
      .map(({ __search_data_index__ }) => ({
